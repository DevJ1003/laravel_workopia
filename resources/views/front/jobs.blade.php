        ...
        <div class="row pt-5">
            <div class="col-md-4 col-lg-3 sidebar mb-4">
                ...
                <div class="card border-0 shadow p-4">
                    ...
                    <div class="mb-4">
                        <h2>Job Type</h2>
                        @if ($jobNatures->isNotEmpty())
                                @foreach ($jobNatures as $jobNature)
                                    <div class="form-check mb-2"> 
                                        <input {{ (in_array($jobNature->id,$jobNatureArray)) ? 'checked' : '' }} class="form-check-input" name="job_nature" type="checkbox" value="{{ $jobNature->id }}" id="job-type-{{ $jobNature->id }}">    
                                        <label class="form-check-label" for="job-type-{{ $jobNature->id }}">{{ $jobNature->name }}</label>
                                    </div>
                                @endforeach
                            @endif
                    </div>
...
<script>
    $("#searchForm").submit(function(e){
        ...
        var checkedJobNatures = $("input:checkbox[name='job_nature']:checked").map(function(){
            return $(this).val();
        }).get();
        ...
        // If user has checked checkedJobNatures
        if (checkedJobNatures.length > 0){
            url += '&jobNature=' +checkedJobNatures;
        }
